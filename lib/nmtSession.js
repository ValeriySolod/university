...
// Clamps an elapsed-time value to the inclusive 0..durationMs range, so a
// persisted expired session submitted long after its deadline (or a
// negative/out-of-order timestamp pair) can never report more than the
// session's actual duration.
export function clampElapsedMs(elapsedMs, durationMs = NMT_SESSION_DURATION_MS) {
  if (typeof elapsedMs !== "number" || !Number.isFinite(elapsedMs)) return 0;
...
export function buildSessionResult({ questions, answersById, startedAtMs, submittedAtMs }) {
  ...
  return {
    ...
    elapsedMs: clampElapsedMs(submittedAtMs - startedAtMs),
  };
}
