...
describe("buildSessionResult", () => {
  ...
  it("preserves the actual elapsed time for a manual submission before expiry", () => {
    const result = buildSessionResult({
      questions: nmtQuestions,
      answersById: {},
      startedAtMs: 0,
      submittedAtMs: NMT_SESSION_DURATION_MS - 60_000,
    });
    expect(result.elapsedMs).toBe(NMT_SESSION_DURATION_MS - 60_000);
  });

  it("reports exactly the session duration for a submission exactly at the deadline", () => {
    const result = buildSessionResult({
      questions: nmtQuestions,
      answersById: {},
      startedAtMs: 0,
      submittedAtMs: NMT_SESSION_DURATION_MS,
    });
    expect(result.elapsedMs).toBe(NMT_SESSION_DURATION_MS);
  });

  it("caps elapsed time at the session duration for a submission after the deadline", () => {
    const result = buildSessionResult({
      questions: nmtQuestions,
      answersById: {},
      startedAtMs: 0,
      submittedAtMs: NMT_SESSION_DURATION_MS + 3 * 60 * 60 * 1000,
    });
    expect(result.elapsedMs).toBe(NMT_SESSION_DURATION_MS);
  });
});
...
describe("clampElapsedMs", () => {
  it("preserves an elapsed time within range", () => {
    expect(clampElapsedMs(90_000)).toBe(90_000);
  });

  it("caps elapsed time at the session duration", () => {
    expect(clampElapsedMs(NMT_SESSION_DURATION_MS + 1)).toBe(NMT_SESSION_DURATION_MS);
    expect(clampElapsedMs(NMT_SESSION_DURATION_MS)).toBe(NMT_SESSION_DURATION_MS);
  });

  it("clamps negative elapsed time to zero", () => {
    expect(clampElapsedMs(-1000)).toBe(0);
  });

  it("returns zero for non-finite or non-numeric input", () => {
    expect(clampElapsedMs(NaN)).toBe(0);
    expect(clampElapsedMs(undefined)).toBe(0);
    expect(clampElapsedMs("not-a-number")).toBe(0);
    expect(clampElapsedMs(Infinity)).toBe(0);
    expect(clampElapsedMs(-Infinity)).toBe(0);
  });

  it("always returns a finite value within 0..NMT_SESSION_DURATION_MS", () => {
    const inputs = [NaN, undefined, "not-a-number", Infinity, -Infinity, -1000, 0, 90_000, NMT_SESSION_DURATION_MS, NMT_SESSION_DURATION_MS + 1];
    inputs.forEach((input) => {
      const result = clampElapsedMs(input);
      expect(Number.isFinite(result)).toBe(true);
      expect(result).toBeGreaterThanOrEqual(0);
      expect(result).toBeLessThanOrEqual(NMT_SESSION_DURATION_MS);
    });
  });
});
