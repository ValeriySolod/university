...
describe("buildPracticeQuestions — unique session ids across combined batches", () => {
  ...
  it("never reuses a generated question's id across independently generated sessions", () => {
    // Regression test: withSessionIds used to number generated questions
    // purely by position ("practice-powers-1"), so two separate sessions
    // that each generated a different random question at the same index
    // produced the same id — collapsing them into one bookmark identity.
    const first = buildPracticeQuestions({ categoryId: "powers", difficulty: "basic", quantity: 4 });
    const second = buildPracticeQuestions({ categoryId: "powers", difficulty: "basic", quantity: 4 });
  ...
  it("keeps the fixed elementary bank's original, stable ids untouched", () => {
    const { questions } = buildPracticeQuestions({ categoryId: "elementary", difficulty: "all", quantity: 10 });
    const originalIds = new Set(elementaryQuestions.map((q) => q.id));
    expect(questions.length).toBeGreaterThan(0);
    expect(questions.every((q) => originalIds.has(q.id))).toBe(true);
  });
});
