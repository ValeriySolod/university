...
const attemptsSlice = createSlice({
  ...
  reducers: {
    ...
    // Clears attempt records only — every other slice (marked questions,
    // topic stats, practice settings, the active NMT session) is untouched.
    attemptsCleared(state) {
      state.items = [];
    },
  },
});

export const { attemptRecorded, attemptsCleared } = attemptsSlice.actions;
