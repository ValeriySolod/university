...
// Retry attempts are recorded like any other attempt, idempotently by id,
// but never feed topicStats: they replay past questions rather than sampling
// fresh ones from a category, so counting them there would double-count and
// skew weak-topic tracking.
export function completeRetrySession({ attempt }) {
  return (dispatch, getState) => {
    const state = getState();
    const alreadyRecorded = state.attempts.items.some((item) => item.id === attempt.id);
    if (alreadyRecorded) return;
