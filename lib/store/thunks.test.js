...
import { completeNmtSession, completePracticeSession, completeRetrySession } from "./thunks";
...
function retryAttempt(id, overrides = {}) {
  return {
    id,
    trainerType: "retry",
    completedAt: 6000,
...
describe("completeRetrySession", () => {
  it("records the retry attempt without touching topic stats", () => {
    const store = makeTestStore();
    store.dispatch(completeRetrySession({ attempt: retryAttempt("retry-1") }));

    expect(store.getState().attempts.items).toHaveLength(1);
    expect(store.getState().attempts.items[0].sourceAttemptId).toBe("practice-1");
    expect(store.getState().topicStats.byCategory).toEqual({});
  });

  it("never duplicates the attempt across repeated dispatches", () => {
    const store = makeTestStore();
    store.dispatch(completeRetrySession({ attempt: retryAttempt("retry-1") }));
    store.dispatch(completeRetrySession({ attempt: retryAttempt("retry-1", { totalPoints: 999 }) }));

    expect(store.getState().attempts.items).toHaveLength(1);
    expect(store.getState().attempts.items[0].totalPoints).toBe(1);
  });
});
