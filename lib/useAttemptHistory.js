...
import { attemptsCleared } from "./store/attemptsSlice";
...
export function useAttemptHistory() {
  ...
  // If the reviewed attempt disappears from underneath the review view
  // (e.g. history cleared), fall back to the list rather than rendering a
  // stale/absent selectedAttempt.
  useEffect(() => {
    if (view === "review" && selectedAttemptId && !selectedAttempt) {
      setSelectedAttemptId(null);
      setRetryMessage(null);
      setView("list");
    }
  }, [view, selectedAttemptId, selectedAttempt]);

  const clearHistory = useCallback(() => {
    dispatch(attemptsCleared());
  }, [dispatch]);
